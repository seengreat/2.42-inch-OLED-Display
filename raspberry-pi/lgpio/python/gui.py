...
class GUI():
    def __init__(self):
        ...
        self.oled.reset()
        self.oled.oled_init()
        self.img = [0xff for i in range(1024)]
        self.mem_w = OLED_WIDTH
        self.mem_h = OLED_HEIGHT
        self.color = BLACK
        self.rotate = ROTATE_90
        self.mirror = MIRROR_HORIZONTAL
        if OLED_WIDTH % 8 == 0:
            self.byte_w = OLED_WIDTH//8
        else:
            self.byte_w = (OLED_WIDTH // 8) + 1
        self.byte_h = OLED_HEIGHT
    ...
    def set_pixel(self, x, y, color):
        ...
        if xx > self.mem_w or yy > self.mem_h:
            ...
            return
        #print(xx,yy)
        addr = xx // 8 + yy * self.byte_w # byte_w=16 byte_h=250
        #print(addr)
        rdata = self.img[addr]
        if color == BLACK:
